    ...
    for sp in ("train", "val", "test"):
        ...
        if g:
            print(f"{sp:<8}{len(g):>7}{max(r['pct_nodata'] for r in g):>10.2f}"
                  f"{np.median([r['pct_burn'] for r in g]):>16.2f}")

    print(f"\nده نمونهٔ بدتر:")
    print(f"{'id':<7}{'split':<8}{'بی‌داده٪':>10}{'سوختگی٪':>10}  {'کاشی':<10}{'تاریخ'}")
    print("-" * 62)
    for r in sorted(bad, key=lambda r: -r["pct_nodata"])[:10]:
        print(f"{r['sample_id']:<7}{r['split']:<8}{r['pct_nodata']:>10.2f}"
              f"{r['pct_burn']:>10.2f}  {r['tile']:<10}{r['date']}")

    # آیا کاشی‌های این نمونه‌ها نمونهٔ سالم هم دارند؟
    bad_tiles = {r["tile"] for r in bad}
    only_bad = [t for t in bad_tiles
                if all(r["pct_nodata"] > NODATA_THR
                       for r in rows if r["tile"] == t)]
    print(f"\nکاشی‌های درگیر: {len(bad_tiles)}")
